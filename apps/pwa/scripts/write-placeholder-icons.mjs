...
/** AccessPortal — AI4Context purple + accessibility symbol (circle + figure) */
...
/** Universal access: outer ring + head + arms arc + legs */
function inAccessibilitySymbol(x, y, W) {
  ...
  const legW = W * 0.045;
  const leftLegCx = cx - legSpread;
  const rightLegCx = cx + legSpread;
  if (y >= legTop && y <= legBot) {
    if (Math.abs(x - leftLegCx) <= legW) return true;
    if (Math.abs(x - rightLegCx) <= legW) return true;
  }
...
for (const size of [16, 48, 128, 192, 512]) {
  fs.writeFileSync(path.join(outDir, `icon${size}.png`), renderIcon(size));
}

console.log('[icons] AccessPortal accessibility PNGs → public/icons');
